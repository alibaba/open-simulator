...
func NewApplier(opts Options) Interface {
	...
	if err != nil {
		log.Fatalf("NewApplier | failed to read config file(%s): %v", opts.SimonConfig, err)
	}
	configJSON, err := yaml.YAMLToJSON(configFile)
	if err != nil {
		log.Fatalf("NewApplier | failed to unmarshal config file(%s) to json: %v", opts.SimonConfig, err)
	}

	if err := json.Unmarshal(configJSON, simonCR); err != nil {
		log.Fatalf("NewApplier | failed to unmarshal config json to object: %v", err)
	}
...
func (applier *Applier) Run() (err error) {
	...
	if len(nodeResource.Nodes) == 0 {
		return fmt.Errorf("Run | the new node directory(%s) has no nodes ", applier.newNode)
	}
...
func validate(applier *Applier) error {
	if len(applier.cluster.KubeConfig) == 0 && len(applier.cluster.CustomCluster) == 0 ||
		...
		return fmt.Errorf("validate | only one of values of both kubeConfig and customConfig must exist ")
	}

	if len(applier.cluster.KubeConfig) != 0 {
		if _, err := os.Stat(applier.cluster.KubeConfig); err != nil {
			return fmt.Errorf("validate | invalid path of kubeConfig: %v ", err)
		}
	}

	if len(applier.cluster.CustomCluster) != 0 {
		if _, err := os.Stat(applier.cluster.CustomCluster); err != nil {
			return fmt.Errorf("validate | invalid path of customConfig: %v ", err)
		}
	}

	if len(applier.schedulerConfig) != 0 {
		if _, err := os.Stat(applier.schedulerConfig); err != nil {
			return fmt.Errorf("validate | invalid path of scheduler config: %v ", err)
		}
	}

	if len(applier.newNode) != 0 {
		if _, err := os.Stat(applier.newNode); err != nil {
			return fmt.Errorf("validate | invalid path of newNode: %v ", err)
		}
	}

	for _, app := range applier.appList {
		if _, err := os.Stat(app.Path); err != nil {
			return fmt.Errorf("validate | invalid path of %s app: %v ", app.Name, err)
		}
	}
...
func newFakeNodes(node *corev1.Node, nodeCount int) ([]*corev1.Node, error) {
	if node == nil {
		return nil, fmt.Errorf("newFakeNodes | node is nil ")
	}
	...
	for i := 0; i < nodeCount; i++ {
		...
		if err != nil {
			return nil, fmt.Errorf("newFakeNodes | %v", err)
		}
	...
	for _, status := range nodeStatuses {
		...
		if nodeStorageStr, exist := node.Annotations[simontype.AnnoNodeLocalStorage]; exist {
			...
			if err := ffjson.Unmarshal([]byte(nodeStorageStr), &nodeStorage); err != nil {
				log.Fatalf("report | error when unmarshal json data, node is %s\n", node.Name)
			}
...
func satisfyResourceSetting(nodeStatuses []simulator.NodeStatus) (bool, string, error) {
	...
	if str := os.Getenv(simontype.EnvMaxCPU); str != "" {
		if maxcpu, err = strconv.Atoi(str); err != nil {
			return false, "", fmt.Errorf("satisfyResourceSetting | convert env %s to int failed: %s ", simontype.EnvMaxCPU, err.Error())
		}
	...
	if str := os.Getenv(simontype.EnvMaxMemory); str != "" {
		if maxmem, err = strconv.Atoi(str); err != nil {
			return false, "", fmt.Errorf("satisfyResourceSetting | convert env %s to int failed: %s ", simontype.EnvMaxMemory, err.Error())
		}
	...
	if str := os.Getenv(simontype.EnvMaxVG); str != "" {
		if maxvg, err = strconv.Atoi(str); err != nil {
			return false, "", fmt.Errorf("satisfyResourceSetting | convert env %s to int failed: %s ", simontype.EnvMaxVG, err.Error())
		}
	...
	for _, status := range nodeStatuses {
		...
		if nodeStorageStr, exist := node.Annotations[simontype.AnnoNodeLocalStorage]; exist {
			var nodeStorage utils.NodeStorage
			if err := ffjson.Unmarshal([]byte(nodeStorageStr), &nodeStorage); err != nil {
				return false, "", fmt.Errorf("satisfyResourceSetting | error when unmarshal json data, node is %s\n", node.Name)
			}
			for _, vg := range nodeStorage.VGs {
	...
	if cpuOccupancyRate > maxcpu {
		return false, fmt.Sprintf("satisfyResourceSetting | the average occupancy rate(%d%%) of cpu goes beyond the env setting(%d%%)\n", cpuOccupancyRate, maxcpu), nil
	}
	if memoryOccupancyRate > maxmem {
		return false, fmt.Sprintf("satisfyResourceSetting | the average occupancy rate(%d%%) of memory goes beyond the env setting(%d%%)\n", memoryOccupancyRate, maxmem), nil
	}
	...
	if totalVGResource.Capacity != 0 {
		...
		if vgOccupancyRate > maxvg {
			return false, fmt.Sprintf("satisfyResourceSetting | the average occupancy rate(%d%%) of vg goes beyond the env setting(%d%%)\n", vgOccupancyRate, maxvg), nil
		}
	}

...
func (applier *DefaultApplier) Run() (err error) {
	resourceList := make(map[string]simontype.ResourceInfo)
	...
	for _, app := range applier.appList {
		...
		resourceList[app.Name] = newResource
	}
	...
	// resourceList confirmation
	var surveyOptions []string
	for _, resourceInfo := range resourceList {
		surveyOptions = append(surveyOptions, resourceInfo.Name)
	}
	...
	err = survey.Ask(multiQs, &surveyAnswers)
	if err != nil {
		log.Fatal(err.Error())
	}
	...
	for i := 0; i < 100; i++ {
		f := func() (error, bool) {
			...
			for _, name := range surveyAnswers {
				success = false
				// synchronize pods generated by deployment、daemonset and like this, then format all unscheduled pods
				appPods := simulator.GenerateValidPodsFromResources(sim.GetFakeClient(), resourceList[name].Resource)
				if !added {
				...
				fmt.Printf(utils.ColorCyan+"%s: %d pods to be simulated, %d pods of which to be scheduled\n"+utils.ColorReset, resourceList[name].Name, len(appPods), utils.GetTotalNumberOfPodsWithoutNodeName(appPods))
				err = sim.SchedulePods(appPods)
				if err != nil {
					fmt.Printf(utils.ColorRed+"%s: %s\n"+utils.ColorReset, resourceList[name].Name, err.Error())
					break
				} else {
					success = true
					fmt.Printf(utils.ColorGreen+"app %s: Success!\n"+utils.ColorReset, resourceList[name].Name)
					// if err := sim.CreateConfigMapAndSaveItToFile(simontype.ConfigMapFileName); err != nil {
					// 	return err
					// }
					// if applier.interactive {
					// 	prompt := fmt.Sprintf("%s scheduled succeessfully, continue(y/n)?", resourceList[name].Name)
